...
def run_events(dev, csv_path=None):
    ...
    if csv_file:
        csv_file.write("t_ms,key,event,travel,flag,off_ms\n")
    ...
    last = {}  # key_idx -> (t16, pressed, host_monotonic) del ultimo evento
    counts = {k: {"press": 0, "release": 0, "phantom": 0, "fast": 0, "trav": [], "offwin": []} for k in EVLOG_KEYS}
    t_status = 0.0
    ...
    try:
        while True:
            ...
            if now_s - t_status > 0.4:
                ...
                print(f"\r  eventos: {tot:6d} | candidatos: {ph:4d} | rapidos(info): {fa:4d}",
                      end="", flush=True)
            ...
            for t, key_idx, pressed, travel in events:
                ...
                if pressed:
                    ...
                    if key_idx in last:
                        ...
                        if lp == 0 and host_dt_ms < OFFWIN_CAP_MS + 1000:
                            if dt < OFFWIN_CAP_MS:
                ...
                if flag:
                    ...
                    print(f"\n{line}   <<< {flag}")
                if csv_file:
                    csv_file.write(f"{t},{name},{'press' if pressed else 'release'},{travel},{flag},{off_ms}\n")
    ...
    if any(c["offwin"] for c in counts.values()):
        print(f"\nVentanas OFF (release→re-press, taps <{OFFWIN_CAP_MS} ms):")
